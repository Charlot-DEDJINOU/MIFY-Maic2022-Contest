...
def try_again(state,occasions,adverse_occasions,double,player) :
    ...
    for occasion in occasions :
        ...
        if len(set(cells) & set(adverse_occasions)) == 0 and len(set(cells) & set(double)) == 0 :
            return occasion
    return False
...
def steal_move(rules,state,player) :
    ...
    if is_mill == True :
        ...
        at,to=ordinateur(state,rules,[],player,[])
        if at != False :
            return at
...
def Move(state,rules,actions,player_current,player) :
   ...
   if player == player_current :
        ...
        if len(actions_mill) == 0 :
            ...
            at,to = ordinateur(state,rules,[],-1*player,[])
            if  at == False and free(state,rules,actions_mill[0].get_action_as_dict()["action"]["to"],-1*player) == True :
    ...
    if actions[0].get_action_as_dict()["action_type"] == MorabarabaActionType.ADD :
        ...
        if len(get_actions_can_make_mill(player*-1,state,rules)) == 0 :
           ...
           if at != False :
                ...
                choice=filter(state,[action for action in actions if action.get_action_as_dict()["action"]["at"] in pieces ],player)
           ...
           return MorabarabaRules.random_play(state,player)
            ...
            choice=[action for action in actions if action.get_action_as_dict()["action"]["at"] in choice]
            choice=filter(state,choice,player)
            if len(choice) == 0 :
                choice=filter(state,new_actions,player)
...
def filter(state,actions,player):
    board = state.get_board()
    for action in actions:
        if len(actions) > 1 :
            adv_pot_mills = get_piece_mills(state,-player,2)
            for move in get_effective_cell_moves(board,action.get_action_as_dict()['action']['at']):
                for mill in adv_pot_mills:
                    if move == mill[2] and  is_effective_making_mill(state,-player,MorabarabaRules,move,mill) == True  and action in actions:
                        actions.remove(action)
            board.empty_cell(action.get_action_as_dict()['action']['at'])
            board.fill_cell(action.get_action_as_dict()['action']['to'],Color(player))
            state.set_board(board)
            at,to = ordinateur(state,MorabarabaRules,[],-player,[])
            if at != False and to ==  action.get_action_as_dict()['action']['at'] and action in actions:
                actions.remove(action)
            board.empty_cell(action.get_action_as_dict()['action']['to'])
            board.fill_cell(action.get_action_as_dict()['action']['at'],Color(player))
            state.set_board(board)
    rest_pieces = [action.get_action_as_dict()['action']['at'] for action in actions]
    return rest_pieces
