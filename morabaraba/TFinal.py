...
def ADD_block(state,player,actions) :
    ...
    if len(cells) == 0 :
         ...
         my_feature_double=double(state,-1*player)
         print(my_feature_double)
         if len(cells) > 1 :
            choices=[cell for cell in set(my_feature_double) & set(my_potential_occasion[2]) & set(cells)]
            if len(choices)  == 0 :
                print("\n\n 1")
            ...
            if len(choices)  == 0 :
                print("\n\n 4")
                if len(my_occasions) > 1 :
                    choices=[cell for cell in my_occasions if get_effective_cell_moves(board,cell) != 0]
                else :
                    choices=[cell for cell in set(my_occasions) & set(cells)]
            if len(choices)  == 0 :
                choices=cells
...
def ADD_and_Ocassion(state,player) :
    ...
    if state.get_player_info(player)["in_hand"] <= 3 and adverse_potential_occasion[0] < 2 and my_potential_occasion[0] < 2  :
        adverse_cells=try_again(state,player)
        if my_occasions and adverse_cells :
            block_and_my_occasion=[cell for cell in set(my_occasions) & set(adverse_cells)]
            if block_and_my_occasion and block_and_my_occasion[0] != False :
                choice=[block_and_my_occasion[0]]
        if len(choice) == 0 and adverse_cells :
            choice=[adverse_cells[0]]
    ...
    if len(choice) == 0 :
        if len(my_occasions) == 0 and len(adverse_occasions) == 0 :
            ...
            if my_potential_occasion[0] >= adverse_potential_occasion[0] and my_potential_occasion[0] != 1 :
                ...
                choice=[adverse_potential_occasion[1]]
            elif adverse_potential_occasion[0] > my_potential_occasion[0] and adverse_potential_occasion[0] !=1 :
                if my_potential_occasion[0] == 2 :
                    choice=[my_potential_occasion[1]]
                else :
...
def double(state,player) :
    cells=[]
    board=state.get_board()
    luck_preced,occasions=Occasion(state,player)
    for cell in board.get_all_empty_cells() :
        board.fill_cell(cell,Color(player))
        state.set_board(board)
        luck,occasions=Occasion(state,player)
        if  len(set(luck[2]) - set(luck_preced[2])) != 0 and (len(luck[2]) > 1 or len(set(luck[2]) - set(cells_make_mill(board,player))) != 0) :
            cells.append(cell)
        board.empty_cell(cell)
    return cells
...
def Move(state,rules,actions,player_current,player) :
   ...
   if player == player_current :
        ...
        if len(actions_mill) == 0 :
            ...
            actions_copie=actions_mill
            if len(get_actions_can_make_mill(-1*player,state,rules)) != 0 :
                cells=which_move(state,rules,actions,player)
                actions_mill=[action for action in actions_mill if action.get_action_as_dict()['action']['at'] in cells]

            if len(actions_mill) > 1 :
                ...
                luck,pity=move_and_occasion(actions_copie,state,rules,player)
                if len(luck) != 0 :
                    return luck[0]
                elif len(pity) != 0 :
                    return pity[0]
